<template>
   <UModal :modelValue="isDialogOpen" :ui="{
      ...
      <UForm :schema="otherSchema" :state="formGroup" @submit="onSubmit" v-on:error="handleFormError"
         class="p-6">
         <img :src="BoxVariant" alt="icon" class="w-12 h-12" />
         <div class="mt-5">
            <div class="text-grayy-400 text-lg font-anuphan-semi-bold mb-1">{{ $t("เพิ่มรายการอื่นๆ") }}</div>
         ...
         <div class="mt-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-4">
               <SelectMenu :label="$t('วัตถุประสงค์')" name="purposeTypeId" :placeholder="$t('วัตถุประสงค์')"
                  :options="documentOptionsStore.purposeTypeOptions.filter((item: any) => item.id === 6)"
...
const props = defineProps({
   ...
   selectedOther: Object,
   selectedOtherIndex: Number,
});
   ...
   (newValue) => {
      if (newValue) {
         ...
         if (props.actionMode === "edit") {
            formGroup.value = JSON.parse(JSON.stringify(props.selectedOther));
         } else {
            resetForm();
         }
      }
...
async function onSubmit(
   event: FormSubmitEvent<z.output<typeof otherSchema>>
) {
   const newItem = {
      id: 0,
      purposeTypeName: "Other",
      countryId: 0,
      countryName: "",
      cargoTypeId: 0,
      cargoTypeName: "",
   ...
   if (props.actionMode === "create") {
      ...
      props.dialogConfirm!("edit", formGroup.value.id ? formGroup.value : newItem, props.selectedOtherIndex);
   } else if (props.actionMode === "delete") {
      newItem.is_delete = true;
      props.dialogConfirm!("delete", newItem, props.selectedOtherIndex);
   }
  }
