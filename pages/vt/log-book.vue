   ...
   <div>
      <div id="log-book-form-content" class="bg-white rounded-[20px]">
         <div class="flex items-center justify-between py-4 px-6 bg-primaryy-500 rounded-t-[20px]">
            ...
            <div>
               <UButton class="bg-white hover:bg-white text-grayy-400 [&_span]:font-anuphan-semi-bold mr-2" outlined
                  icon="i-heroicons-plus-circle" size="md" label="เพิ่มเรือราชการ"
      ...
      <DialogConfirm :title="$t('delete_logbook')" :desc="$t('confirm_delete_logbook')" icon="danger"
...
const { getVtmsLogBook, getBerth, deleteVtmsLogBook } = logBookService();
const { t } = useI18n();
...
const filter = ref({
   keyword: "", berth_id: "", status: "", offset: 0, page: 1, limit: 10, Order: [{ column: sortField.value, dir: sortOrder.value }]
});
      ...
      [
         {
            ...
            click: () => {
               if (row.agency_name == "VTMS") {
                  navigateTo(`/vtms/log-book/government/edit?id=${row.id}`);
               } else {
                  navigateTo(`/vtms/log-book/edit?id=${row.id}`);
               }
            },
         },
         {
...
const dialogConfirmDelete = async () => {
   isLoading.value = true;
   if (rowId) {
      const resp = await deleteVtmsLogBook(rowId.value);
      if (resp.statusCode === 200) {
         isAlertDialogActivateOpen.value = true;
         modalTitleDialog.value = t("delete_logbook");
         modalAlertMessage.value = t("delete_logbook_success");
