<template>
      ...
      <div id="request-form-content" class="bg-white rounded-[20px]">
         <div class="flex items-center justify-between py-5 px-6 bg-primaryy-500 rounded-t-[20px]">
            <div class="flex justify-between">
               <h1 class="font-normal text-base text-grayy-25">{{ $t("berthing_purpose") }}</h1>
            ...
            <div>
               <UButton
                  ...
                  size="md"
                  :label="$t('add')"
                  @click="() => handlePurposeForm(true, 'create')"
               />
            </div>
         </div>
         <UTable
   ...
   <PurposeForm
      :isDialogOpen="isDialogPurposeFormOpen"
      :dialogCancel="dialogPurposeCancel"
      :dialogConfirm="dialogPurposeConfirm"
      :mode="actionMode"
      :itemId="purposeData?.purpose_id"
   />
...
import PurposeForm from "~/components/dialog/master-data/port/purpose-form.vue";
...
const { getPurpose, updatePurpose } = usePurposeService();
...
const sortField = ref("");
const sortOrder = ref("");
const filter = ref<IFilterPurpose>({
   limit: 10,
   page: 1,
   search: "",
   status: "",
...
const getPurposeList = async () => {
   ...
   const resp = await getPurpose(filter.value);
   if (resp.statusCode === 200) {
      purposeList.value = resp.data.data;
...
const purposeData = ref<IPurposeGet | null>(null);
const getMenuTable = (row: IPurposeGet) => {
   const { status } = row;
   const items = [
      [
         {
            ...
            click: () => {
               purposeData.value = row;
               handlePurposeForm(true, "edit");
            },
         },
         {
...
const updatePurposeStatus = async () => {
   const body = {
      ...purposeData.value!,
      status: !purposeData.value?.status,
   };

   isLoading.value = true;
   const resp = await updatePurpose(purposeData.value?.purpose_id!, body);
   if (resp.statusCode === 200) {
      handleResetFilter();
      ...
      console.error("update purpose error:", resp);
...
const isDialogPurposeFormOpen = ref(false);
const handlePurposeForm = (status: boolean, mode: string) => {
   actionMode.value = mode;
   isDialogPurposeFormOpen.value = status;
};
const dialogPurposeCancel = () => {
   isDialogPurposeFormOpen.value = false;
};
const dialogPurposeConfirm = (status: boolean, type: string = "") => {
   isDialogPurposeFormOpen.value = status;
   if (type === "refresh") {
      filter.value.page = 1;
      getPurposeList();
...
const selectRow = (row: IPurposeGet) => {
