<template>
   <div>
      <div id="filter-search" class="bg-purple-25 py-4 px-6 rounded-xl mb-6">
         <div class="grid grid-cols-9 gap-4">
            <div class="col-span-3">
               <Input :label="$t('search_title')" name="keyword" v-model="filter.keyword"
                  :placeholder="$t('search_title_placeholder')" icon="i-heroicons-magnifying-glass-20-solid" />
            </div>
            <div class="col-span-2">
               <UFormGroup :label="$t('search_status')">
                  <USelectMenu class="max-w-[512px]" color="purple" variant="outline" v-model="filter.status"
                     value-attribute="status" option-attribute="text" :options="statusList"
                     :placeholder="$t('search_status_placeholder')" :ui="{
                        base: 'h-[36px] bg-white',
                        wrapper: 'rounded-md bg-white',
                     }" @change="() => getVesselFlagList()" />
               </UFormGroup>
            </div>
      ...
      <div id="request-form-content" class="bg-white rounded-[20px]">
         <div class="flex items-center justify-between py-4 px-6 bg-primaryy-500 rounded-t-[20px]">
            <div class="flex justify-between">
               <h1 class="font-normal text-lg text-white">{{ $t("vessel_flag_topic") }}</h1>
               <div class="rounded-full bg-purple-25 text-sm py-1 px-2 ml-2">{{ total }} {{ $t("vessel_flag_total") }}
               </div>
            </div>
            <div>
               <VesselDialog @fetch="getVesselFlagList">
                  <template #activator="{ on }">
                     <UButton class="bg-white hover:bg-white text-grayy-400 [&_span]:font-anuphan-semi-bold" outlined
                        icon="i-heroicons-plus-circle" size="md" label="เพิ่ม" v-on="on" />
                  </template>
               </VesselDialog>
            </div>
         </div>
         <UTable @select="selectRow" :columns="columns" :rows="vesselFlagList" :ui="{
            base: 'u-table-list',
         }">
            ...
            <template #path-data="{ row }">
               <img v-if="row.path" :src="row.path" alt="" class="w-[25px] h-[25px] rounded-full" />
            </template>
      ...
      <VesselDialog edit-mode @fetch="getVesselFlagList" class="flex justify-center" hide-button :vessel-id="rowId"
         :data="rowData" :set-open="isOpenForm" @close="setDialogEdit(false)" />
      <DialogConfirm :title="`${rowData?.status ? 'ปิด' : 'เปิด'}การใช้งาน`"
         :desc="`คุณต้องการ${rowData?.status ? 'ปิด' : 'เปิด'}ใช่หรือไม่`" icon="warning" :isDialogOpen="isOpenStatus"
         :dialogCancel="dialogCancel" :dialogConfirm="() => activateVesselFlag(rowData?.status)" />
...
import VesselDialog from "~/components/master-data-form/vesselFlag.vue";
...
const { getVesselFlag, putVesselFlag } = masterDataShipService();
const sortField = ref("");
const sortOrder = ref("");
const columns = [
   {
      key: "id",
      label: t("vessel_flag_table_order"),
      class: "w-[60px] text-center",
   },
   {
      key: "status",
      label: t("vessel_flag_table_status"),
      class: "w-[97px] text-center",
   },
   {
      key: "path",
      label: t("vessel_flag_table_flag"),
      class: "w-[70px] text-center",
   },
   {
      key: "name",
      label: t("vessel_flag_table_nationality"),
   },
   {
      key: "updated_at",
      label: t("vessel_flag_table_update_date"),
   },
   {
      key: "updated_by",
      label: t("vessel_flag_table_update_by"),
   },
   {
      key: "actions",
      sortable: false,
...
const filter = ref({
   keyword: "", status: "", offset: 0, page: 1, limit: 10, Order: [{ column: sortField.value, dir: sortOrder.value }]
});
...
const vesselFlagList = ref([]);
const total = ref(0);
const isDialogVesselOpen = ref(false);

onMounted(() => {
   getVesselFlagList();
});
const getVesselFlagList = async () => {
   try {
...
const openVesselFlagDialog = async () => {
   isDialogVesselOpen.value = true;
};
const activateVesselFlag = async (status) => {
   try {
      isLoading.value = true;
      let resp = await putVesselFlag(rowId.value, { ...rowData.value, status: !status });
      if (resp.statusCode === 200) {
...
const filteredColumns = computed(() => columns.filter((column) => !["id", "path", "actions"].includes(column.key)));
