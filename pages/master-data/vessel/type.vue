<template>
   <div>
            ...
            <div class="col-span-2">
               <UFormGroup :label="$t('search_status')">
                  <USelectMenu class="max-w-[512px]" color="purple" variant="outline" v-model="filter.status"
                     value-attribute="status" option-attribute="text" :placeholder="$t('search_status_placeholder')"
                     :options="statusList" :ui="{
                        base: 'h-[36px] bg-white',
                        wrapper: 'rounded-md bg-white',
                     }" @change="() => getVesselTypeList()" />
               </UFormGroup>
            </div>
      ...
      <div id="request-form-content" class="bg-white rounded-[20px]">
         <div class="flex items-center justify-between py-4 px-6 bg-primaryy-500 rounded-t-[20px]">
            <div class="flex justify-between">
               <h1 class="font-normal text-lg text-white">{{ $t("vessel_type_table_type") }}</h1>
               <div class="rounded-full bg-purple-25 text-sm py-1 px-2 ml-2">{{ total }} {{ $t("vessel_type_total") }}
               </div>
            </div>
            <div>
               <VesselDialog @fetch="getVesselTypeList">
         ...
         <UTable @select="selectRow" :columns="columns" :rows="vesselTypeList" :ui="{
            base: 'u-table-list',
         }">
            ...
            <template #status-data="{ row }">
               <Badge
                  :label="row.status === true ? $t('vessel_type_table_status_activate') : $t('vessel_type_table_status_disable')"
      ...
      <VesselDialog edit-mode @fetch="getVesselTypeList" class="flex justify-center" hide-button :vessel-id="rowId"
      ...
      <DialogConfirm :title="`${rowData?.status ? 'ปิด' : 'เปิด'}การใช้งาน`"
         ...
         :dialogCancel="dialogCancel" :dialogConfirm="() => activateVesselType(rowData?.status)" />
...
import VesselDialog from "~/components/master-data-form/vesselType.vue";
...
const { getVesselType, putVesselType } = masterDataShipService();
const sortField = ref("");
const sortOrder = ref("");
const columns = [
   {
      key: "id",
      label: t("vessel_type_table_order"),
      class: "w-[60px] text-center",
   },
   {
      key: "status",
      label: t("vessel_type_table_status"),
      class: "w-[97px] text-center",
   },
   {
      key: "name",
      label: t("vessel_type_table_type"),
   },
   {
      key: "updated_at",
      label: t("vessel_type_table_update_date"),
   },
   {
      key: "updated_by",
      label: t("vessel_type_table_update_by"),
   },
   {
      key: "actions",
      sortable: false,
...
const getMenuTable = (row) => {
   const { status, name, created_at, created_by, id, updated_at, updated_by } = row;
   const items = [
      [
         {
            label: "แก้ไข",
            ...
            click: () => {
               rowData.value = { name: name, status: status };
               rowId.value = row.id;
               setDialogEdit(true);
            },
         },
...
const openVesselTypeDialog = async () => {
   isDialogVesselOpen.value = true;
};
const activateVesselType = async (status) => {
   try {
      isLoading.value = true;
      let resp = await putVesselType(rowId.value, { ...rowData.value, status: !status });
      if (resp.statusCode === 200) {
