<template>
   <div>
      ...
      <div id="request-form-content" class="bg-white rounded-[20px]">
         <div class="flex items-center justify-between py-5 px-6 bg-primaryy-500 rounded-t-[20px]">
            <div class="flex justify-between">
               <h1 class="font-normal text-lg text-white">{{ $t("product_type_topic") }}</h1>
               <div class="rounded-full bg-purple-25 text-sm py-1 px-2 ml-2">{{ total }} {{ $t("product_type_total") }}</div>
            </div>
            <div>
               <ProductTypeDialog @fetch="getProductTypeList">
         ...
         >
            ...
            <template #product_type_id-data="{ index }">
            ...
            <template #status-data="{ row }">
               <Badge
                  :label="row.status === true ? $t('product_type_table_status_activate') : $t('product_type_table_status_disable')"
      ...
      <ProductTypeDialog
         edit-mode
         @fetch="getProductTypeList"
         class="flex justify-center"
         hide-button
         :product-type-id="rowId"
         :data="rowData"
         :set-open="isOpenForm"
      ...
      <DialogConfirm
         ...
         :dialogConfirm="() => activateProductType(rowData?.status)"
      />
...
import ProductTypeDialog from "~/components/dialog/master-data/product/product-type-form.vue";
...
const { t } = useI18n();
const { getProductType, updateProductType } = masterDataShipService();
const sortField = ref("");
const sortOrder = ref("");
const columns = [
   {
      key: "product_type_id",
      label: t("product_type_table_order"),
      class: "w-[60px] text-center",
   },
   {
      key: "status",
      label: t("product_type_table_status"),
      class: "w-[97px] text-center",
   },
   {
      key: "product_group_name",
      label: t("product_type_table_group_name"),
      class: "w-[70px] text-center",
   },
   {
      key: "product_type_name",
      label: t("product_type_table_type_name"),
      class: "w-[70px] text-center",
   },
   {
      key: "updated_at",
      label: t("product_type_table_update_date"),
   },
   {
      key: "updated_by",
      label: t("product_type_table_update_by"),
   },
   {
      key: "actions",
      sortable: false,
...
const getMenuTable = (row) => {
   const { product_type_id, status, name, created_at, created_by, id, updated_at, updated_by } = row;
   const items = [
      [
         {
            label: "แก้ไข",
            ...
            click: () => {
               ...
               rowId.value = row.product_type_id;
               setDialogEdit(true);
            },
         },
         {
...
const getProductTypeList = async () => {
   try {
      ...
      const resp = await getProductType(queryString);
      if (resp.statusCode === 200) {
         ...
         console.log("productTypeList.value", productTypeList.value);
      }
   } catch (error) {
      console.error(error);
   } finally {
...
const activateProductType = async (status) => {
   try {
      isLoading.value = true;
      let resp = await updateProductType(rowId.value, { ...rowData.value, status: !status });
      if (resp.statusCode === 200) {
...
const filteredColumns = computed(() => columns.filter((column) => !["product_type_id"].includes(column.key)));
