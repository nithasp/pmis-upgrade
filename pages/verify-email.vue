<template>
   <main class="relative h-screen bg-[url('~/assets/images/login/bg.jpg')] bg-cover bg-center">
      ...
      <div :class="`h-screen flex p-10 items-center justify-center overflow-auto`">
         <div class="w-1/2">
            <div class="rounded-[20px] bg-white m-auto p-[24px] max-w-[414px]">
               <UForm :state="formVerify" @submit="onSubmit">
                  ...
                  <div class="text-center flex flex-col items-center">
                     ...
                     <h1 class="font-bold text-[30px] my-3">{{ $t('create_an_account') }}</h1>
                     <p v-if="!isSendEmail" class="mb-6 text-grayy-75">{{ $t('register_sub_message') }}</p>
                  </div>
                  <div v-if="!isSendEmail">
                     <Input :label="$t('email')" name="email" class="mb-5" placeholder="me@example.com"
                        v-model="formVerify.email" customIcon="user" @blur="validate('email')"
...
const { sendEmail } = useAuthService();
...
const validate = (field: string) => {
   ...
   if (field === 'email') {
      ...
      } else if (!regxEmail.test(formVerify.email)) {
